...
public class S3URLConnection extends URLConnection {
	...
	public InputStream getInputStream() throws IOException {
		...
		try {
			...
			final String bucketName = this.url.getPath().substring(0,
				this.url.getPath().indexOf("/"));
			final String key = this.url.getPath().substring(bucketName.length() + 1);
			...
			final byte[] file = new byte[(int) s3object.getObjectMetadata().getContentLength()];
			int readCount = 0;
